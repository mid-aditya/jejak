...
const AnalyticsPage: NextPage = () => {
  ...
  const handleExport = () => {
    toast.success('Mengekspor data analitik...');
  };

  return (
    <>
      <Head><title>Analisis - Jejak</title></Head>
      <Layout>
        <div className="space-y-6 animate-fade-in">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Analisis Bisnis</h1>
              <p className="text-gray-500 mt-1">Metrik pendapatan, pertumbuhan, dan konversi</p>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <StatCard icon={<CurrencyDollarIcon className="w-6 h-6" />} label="Total Pendapatan" value="Rp 125M" trend={{ value: 22.1, isUp: true }} color="primary" />
            <StatCard icon={<UserGroupIcon className="w-6 h-6" />} label="Total Pengguna" value="12.450" trend={{ value: 15.2, isUp: true }} color="secondary" />
            <StatCard icon={<MapIcon className="w-6 h-6" />} label="Total Pemesanan" value="3.420" trend={{ value: 8.5, isUp: true }} color="accent" />
            <StatCard icon={<ArrowTrendingUpIcon className="w-6 h-6" />} label="Tingkat Konversi" value="6,4%" trend={{ value: 2.1, isUp: true }} color="primary" />
          </div>

          {/* Charts Row 1 */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="card p-6">
              <h2 className="font-semibold text-gray-900 mb-4">Tren Pendapatan</h2>
              <div className="h-64"><Line data={revenueData} options={chartOptions} /></div>
            </div>
            <div className="card p-6">
              <h2 className="font-semibold text-gray-900 mb-4">Pertumbuhan Pengguna</h2>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="card p-6">
              <h2 className="font-semibold text-gray-900 mb-4">Tren Pemesanan</h2>
              <div className="h-64">
                <Line data={bookingTrendsData} options={{ ...chartOptions, plugins: { legend: { display: true, position: 'bottom' as const } } }} />
              </div>
            </div>
            <div className="card p-6">
              <h2 className="font-semibold text-gray-900 mb-4">Pipa Konversi</h2>
            ...
            <div className="card p-6">
              <h2 className="font-semibold text-gray-900 mb-4">Gunung Populer</h2>
                ...
                {popularMountains.map((m, i) => (
                  <div key={m.name} className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      <p className="text-sm font-medium">{m.trips} trip</p>
